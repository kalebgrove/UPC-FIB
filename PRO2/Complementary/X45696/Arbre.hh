...
private:
  ...
  static bool evaluate_aux(node_arbre *n ){
    if(n->info == "and") {
      return evaluate_aux(n->segE) and evaluate_aux(n->segD);
    }
    else if(n->info == "or") {
      return evaluate_aux(n->segD) or evaluate_aux(n->segE);
    }
    else if(n->info == "not") {
      if(n->segE != nullptr) {
        return not(evaluate_aux(n->segE));
      }
      else {
        return not(evaluate_aux(n->segD));
      }
    }
    else {
      if(n->info == "false") return false;
      return true;
    }
  }
};
