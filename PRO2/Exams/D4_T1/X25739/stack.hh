...
private:
  ...
  Item* copyItems(Item *pitem) {
    ...
    while (pitem != NULL) {
      ...
      pcurrentitem->sumbelow = pitem->sumbelow;
...
public:
  ...
  void push(T value) {
    ...
    if(ptopitem != NULL) pnewitem->sumbelow = ptopitem->sumbelow + value;
    else pnewitem->sumbelow = value;
    ptopitem = pnewitem;
    _size++;
  }
...
T sum() {
  if(ptopitem != NULL) return ptopitem->sumbelow;
  return T();
}
